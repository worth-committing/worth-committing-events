...
class AdminLinkAjax {
  ...
  public function __construct() {
    // If we are in the admin area, load directly from this file,
    // otherwise load from the js file.
    if( is_admin() ) {
      add_action( 'admin_footer', [ $this, 'add_event_link_javascript' ] );
      add_action( 'wp_ajax_add_event_link', [ $this, 'add_event_link' ] );
    } else {
      add_action( 'init', [ $this, 'enqueue_js' ] );
      add_action( 'wp_ajax_nopriv_add_event_link', [ $this, 'add_event_link' ] );
    }
  ...
  public function enqueue_js(): void {
    wp_enqueue_script( 'add-remove-event-link-row', plugin_dir_url( __FILE__ ) . '../scripts/add-event-link.js', [ 'jquery' ], '', TRUE );
  }



  function add_event_link_javascript() { ?>
    <script type="text/javascript" id="add-remove-event-link">
      jQuery(document).ready(function ($) {
        ...
        $("#add_link").click(function () {
          ...
          jQuery.post(ajaxurl, data, function (response) {
            ...
            $("#remove_link").data("delete_row", del_row_num);
            $("#add_link").data("current_row", add_row_num);

            /** Check to see if there is more than one row... **/
            if (add_row_num > 1) {
              // if there is, enable the remove row button.
              $("#remove_link").prop("disabled", false)
            }
        ...
        $("#remove_link").click(function () {
          ...
          const row_num = $("#remove_link").data('delete_row');
          ...
          if (row_num <= 1) {
            ...
            $("#remove_link").prop("disabled", true)
  ...
  #[NoReturn] function add_event_link(): void {
    ...
    // $this->logger->info( "<- --------------------------------------- ->" );
    $new_row_html = (new FormBuilder())->build_event_link_row( NULL, $_POST['current_row'], $_POST['is_admin'] );
